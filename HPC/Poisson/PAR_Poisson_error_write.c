...
void Setup_Grid()
{
  ...
  MPI_Bcast(&relaxation, 1, MPI_FLOAT, 0, grid_comm); /* broadcast max_iter */
...
void Solve()
{
  ...
  int phase  = (offset[X_DIR] + offset[Y_DIR]) % 2;
  Debug("Solve", 0);
  ...
  {
    char filename[64];
    sprintf(filename, "error/X_%i_Y_%i.dat", gridsize[X_DIR], gridsize[Y_DIR]);
    if ((f = fopen(filename, "w")) == NULL)
      Debug("Write_Error fopen failed", 1);
  }
