...
int main(int argc, char **argv)
{
    ...
    for (numberOfElementsToSend = 1; numberOfElementsToSend < (1<<20); numberOfElementsToSend <<= 1)
    {
        double loop_time = 0;
        for (int loop_number = 0; loop_number < 100; loop_number ++)
        {
        ...
        */
                ...
                if (loop_number == 0)
                    loop_time = (endTime - startTime)/10;
                else
                    loop_time = (loop_time < (endTime - startTime)/10) ? loop_time : (endTime - startTime)/10;
    ...
    if (myRank == 0)
        printf("Ping Pong took %f seconds, for %i elements\n", loop_time, numberOfElementsToSend);
    }
    // Finalize MPI
    MPI_Finalize();
